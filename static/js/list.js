...
function CancelFocus(handler,uid,flag) {
    if(flag == 1){ //主动取消关注，警告提示
        temp = '<span class="user-txt-1">举报对方取关并去取关对方，是否确认？</span>' +
                '<span class="user-txt-1">（确认取关时对方会收到被取关的消息，请勿恶意取关）</span>';
    }
    else{
    ...
    layer.open({
        ...
        yes: function () {
            ...
            layer.closeAll();
            $.post('/hufen/cancel', {"otherid":uid}, function (data) { }, "json");
            return false
        },
        btn2:function (index) {
            layer.close(index);
